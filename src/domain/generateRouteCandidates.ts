...
import { buildGoogleMapsEmbedUrl, buildGoogleMapsUrl } from './mapsUrl';
...
function isOpenAt(shop: Shop, arrivalTime?: string) {
 if (!arrivalTime) return shop.openingHours?.openNow !== false;
 const periods = shop.openingHours?.daily;
 if (!periods?.length) return shop.openingHours?.openNow !== false;
