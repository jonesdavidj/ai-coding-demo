...
function defaultConstraints(id) {
    ...
    return { maxExtraWalkMinutes: journey.defaultMaxExtraWalkMinutes, maxStops: journey.defaultMaxStops, openOnly: true, preferFewerStops: true, avoidLowConfidence: true };
}
...
function findRoute() {
    ...
    if (!parsedItems.length)
        return setState({ message: 'Add at least one item to find a route.', routeOptions: [] });
    ...
    setState({ routeOptions: selected, showAlternatives: false, capture: false, message: selected.length ? '' : 'No worthwhile detour found within your time limit.' });
}
...
function render() {
    ...
    root.querySelector('#find-route')?.addEventListener('click', () => { constraints = { ...constraints, maxExtraWalkMinutes: Number(root.querySelector('#max-extra').value), maxStops: Number(root.querySelector('#max-stops').value), openOnly: root.querySelector('#open-only').checked, preferFewerStops: root.querySelector('#prefer-fewer').checked }; findRoute(); });
